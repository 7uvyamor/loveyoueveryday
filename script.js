...
function initHeartCollage() {
  ...
  galleryData.forEach((item, index) => {
    ...
    img.onerror = function () {
      this.src =
        "data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='80' height='80'><rect width='80' height='80' fill='%23ccc'/><text x='50%' y='50%' dominant-baseline='middle' text-anchor='middle' font-size='14'>Pic</text></svg>";
    };
  ...
  if (finalGalleryElement && !hasAttachedFinalDragEvent) {
    ...
    finalGalleryElement.addEventListener("mouseleave", () => {
      isFinalDown = false;
    });
    finalGalleryElement.addEventListener("mouseup", () => {
      isFinalDown = false;
    });
